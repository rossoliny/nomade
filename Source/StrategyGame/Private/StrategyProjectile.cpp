...
void AStrategyProjectile::NotifyActorBeginOverlap(class AActor* OtherActor)
{
	...
	const ABaseCharacter* HitChar = Cast<ABaseCharacter>(OtherActor);
	if (HitChar && HitChar->GetTeamNum() > EStrategyTeam::Unknown && HitChar->GetTeamNum() != GetTeamNum())
	{
		FHitResult PawnHit;
		PawnHit.Actor = MakeWeakObjectPtr(const_cast<ABaseCharacter*>(HitChar));
...
{
	const ABaseCharacter* HitChar = Cast<ABaseCharacter>(HitResult.Actor.Get());
